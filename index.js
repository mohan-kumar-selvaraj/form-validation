...
document.getElementById("cn").addEventListener("input", (event) => {
...
document.getElementById("occupation").addEventListener("input", (event) => {
...
document.getElementById("dob").addEventListener("input", (event) => {
  if (isValidDate(event.target.value)) {
    isTrue("dob-validation", event.target);
    bdob = true;
  } else {
    isFalse("dob-validation", "Date", event.target);
    bdob = false;
  }
});

document.getElementById("gender").addEventListener("input", (event) => {
  isTrue("gender-validation", document.getElementById("gender"));
});

document.getElementById("education").addEventListener("input", (event) => {
  isTrue("education-validation", document.getElementById("education"));
});
...
function submit1(event) {
  if (
    ...
    if (document.getElementById("occupation").value === "")
      data["occupation"] = "";
    else data["occupation"] = document.getElementById("occupation").value;
    // console.log(data);

    saveToLocal(data, document.getElementById("username").value.toLowerCase());
    ...
    .map(
      ...
      <div>
        ...
        <div>
          ...
          <p><strong>DOB:</strong> ${value.dob}</p>
          <p><strong>Contact:</strong> ${value.contact}</p>
          <p><strong>PAN:</strong> ${value.pan}</p>
          <p><strong>Occupation:</strong> ${value.occupation}</p>
          <p><strong>Gender:</strong> ${value.gender}</p>
          <p><strong>Education:</strong> ${value.education}</p>
          <p><strong>Image Size:</strong> ${getImageSize(value)}</p>
        </div>
      </div>
    `
    )
